...
-   基本概念
    ...
    -   [阿里云实时计算Flink版适用哪些场景？](#section_qyr_jsk_fjf)
...
阿里云实时计算Flink版提供标准的Flink SQL语义可以协助您完成流式计算逻辑的处理，但无法满足某些特定场景的业务需求。阿里云实时计算Flink版为部分授信用户提供全功能的UDF函数，帮助授信用户完成业务定制化的数据处理需求。所以，在流数据分析领域，您可以直接使用Flink SQL和UDF即可完成大部分流式数据分析处理逻辑。
...
-   阿里云实时计算Flink版支持华北、华东、华南、新加坡和中国（香港）地域，您可以按区域购买。
