...
-   [Python作业，如果Checkpoint慢怎么办？](#section_0uy_tyo_rp9)
...
-   [如何解析Flink作业所依赖服务的域名？](#section_yb2_8hf_2fh)
-   [报错：undefined](#section_0n6_jx3_4nt)
-   [报错：OSSException](#section_if4_l6i_qkj)
-   [报错：exceeded quota: resourcequota](#section_kwo_lhp_iul)
-   [INFO：org.apache.flink.fs.osshadoop.shaded.com.aliyun.oss](#section_urg_g1f_4ob)
-   [报错：Object '\*\*\*\*' not found](#section_296_t37_i1e)
-   [报错：Only a single 'INSERT INTO' is supported](#section_x07_4xt_s0l)
-   [报错：Exceeded checkpoint tolerable failure threshold](#section_fho_3df_ihq)
...
-   解决方案
    ...
    在作业开发页面单击右侧的**高级配置**后，在**更多Flink配置**项中，设置以下参数，将缓存调小。
...
-   解决方案
    ...
    -   [绑定EIP](/cn.zh-CN/基本功能操作/创建NAT网关实例.md)。
...
-   您没有自建的DNS或者Flink VPC无法与自建的DNS连通。
    ...
    此时，您需要基于阿里云云解析PrivateZone进行域名解析。假如您的Flink VPC为vpc-flinkxxxxxxx，Flink作业需要访问的服务的域名分别为aaa.test.com 127.0.0.1、bbb.test.com 127.0.0.2和ccc.test.com 127.0.0.3。
