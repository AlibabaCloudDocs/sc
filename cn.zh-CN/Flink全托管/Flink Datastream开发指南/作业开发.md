...
-   直接将Connector作为项目依赖打进作业JAR包。
    1.  在Maven项目的pom.xml文件中添加下列配置以引用SNAPSHOT仓库。
    ...
    2.  检查您的Maven配置是否存在`<mirrorOf>*</mirrorOf>`配置。

        如果存在`<mirrorOf>*</mirrorOf>`配置，则需要将此配置改为`<mirrorOf>*,!oss.sonatype.org-snapshot,!apache.snapshots</mirrorOf>`。修改的目的是为了避免SNAPSHOT仓库被覆盖，因为mirrorOf 中使用星号（\*）会导致第一步中配置的两个repository被覆盖。
